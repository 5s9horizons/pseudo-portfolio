...
const ChatBotWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!API_KEY) {
      ...
      return; // Return early if API_KEY is missing
    }
    ...
    const initializeChat = async () => {
      try {
        ...
        if (isMounted.current) {
          ...
          // Add initial bot message ONLY if initialization was successful
          setMessages([{ id: Date.now().toString(), text: CHATBOT_INITIAL_MESSAGE, sender: 'bot', timestamp: Date.now() }]);
          setError(null); // Clear any previous error state upon successful initialization
        }
  ...
  const sendMessage = async (text: string) => {
    if (!chat || isLoading || !text.trim() || error) return; // Also check for error state

    const userMessage: ChatMessage = { id: Date.now().toString(), text, sender: 'user', timestamp: Date.now() };
    setMessages(prev => [...prev, userMessage]);
    setIsLoading(true);
    // setError(null); // Don't clear general errors here, only init errors
    ...
    try {
      ...
      if (isMounted.current) {
        ...
        // setError(errorMessageText); // Optionally set error state for display
      }
    } finally {
      if (isMounted.current) {
        setIsLoading(false);
    ...
    <>
      <div className="fixed bottom-5 right-5 z-50">
        <button
          ...
          disabled={!!error && !isOpen} // Disable FAB if error and chat is closed
        >
      ...
      {isOpen && (
        <ChatWindow
          ...
          onClose={toggleChat}
          title={CHATBOT_NAME}
          error={error} // Pass the error state to ChatWindow
        />
      )}
    </>
  );
